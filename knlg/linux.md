...
* 启动参数: 
    ...
    * 预先执行命令: 如, `gdb-multiarch -ex "set architecture mips" -ex "set endian big"`
* `coredump`: 
...
* **[ false ]** 这个条件的结果是`true`!
* `[[]]`: 是bash等才有的对`[]`的增强. 
    * 可使用布尔操作符(`&&`, `||`)和字符串大小比较符合(`>`, `<`): 比如, `[[ $cond1 && $cond2 ]]`
...
* `ipcs`: 获取进程间通信信息
    ...
    * `-s`: 获取信号量. 
### 文件和目录
* `pushd <dir>`: 将目录压栈, 并切换工作目录到该目录. 
* `popd`: 弹栈, 并切换工作目录到弹出的目录. 
...
* `parted`: 用于创建, 查看, 删除和修改磁盘分区. 它是一个磁盘分区和分区大小调整工具. 这个命令算是对`fdisk`命令的一个补充, 因为如果磁盘大小大于2TB就无法使用fdisk命令进行分区操作了. 
    ...
    * `-l`: 列出所有块设备上的分区布局
    * `-m`: 显示机器可解析的输出
* `mknod <设备路径> <设备类型> <主设备号> <次设备号>`: 创建字符及块设备文件和对应的inode
...
* `grep`
    ...
    * 例子
        ...
        * `grep -dnw my_dir/ --color -E "my pattern|$"`: 在模式中加上`|$`, 可以防止显示结果中不匹配的行被过滤掉. `--color`让匹配项高亮显示. 
* `sort`: 排序
    * `-V`: 对字符串中的(版本号)数字按大小排序. 

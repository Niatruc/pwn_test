...
* 模反元素: 若$a$与$n$互质, 则必有$b$使得$ab \equiv 1 (mod\ n)$, b称为a的模反元素. 此时有 $(ab - 1)$ 被 $n$整除. 
* 欧几里得算法(辗转相除法)
    * 用于求两个非负整数的最大公约数
...
* 欧拉定理: 互质且大于1的两个正整数 $a$ 和 $n$ 存在如下关系: 
    ...
    * 证明(参考: https://bzc.idealand.xyz:8443/w/20727)
        ...
        * 把 $(x_1* ....  *x_f)\%n$ 记为 $k$. 上式两边模 $n$, 再按乘积定理化简, 可得: $$(R * k)\%n = k$$
        * 接下来只要证明在这个等式中, $R$ 必须为1. 因为 $x$ 和 $n$ 互质, 所以 $x1*x2*...xf$ 和 $n$ 互质, 再根据定理1, 所以 $k$ 和 $n$ 互质. 
        * 根据定理2, $k * 1, k * 2, ..., k * n$ 是模 $n$ 的完系. 又因为$(1 * k)\%n = k$, 所以完系中 $1 * k$ 以外的其它数模 $n$ 都不会等于 $k$, 所以 $R$ 为1. 
    * 特例: 费马小定理, 若正整数 $a$ 与质数 $p$ 互质, 则: $$a^{p-1} \equiv 1 (mod\ n)$$
    * 应用
        * 证明模反元素必然存在: $a * a^{F(n) - 1} \% n = 1$

* 定理1: 如果 $a$ 与 $b$ 为互质的大于1的正整数, 则 $b$ 与 $a\%b$ 互质. 
    * 证明: 联想一下辗转相除求最大公约数. 
* 定理2: 如果m, n为互质且大于1的整数, 则$m*1, m*2, ..., m*n$为n的一个完全剩余系. 
    * 证明
        * 用反证法. 若存在$m*a和m*b$模n同余, 则$(m*a - m*b) \% n = 0$ 及 $m(a - b) \% n = 0$
        * 而 $a - b \in [1, n)$ , 参考下面的定理知不成立. 
* 定理3: 如果 $m,n$ 为互质的大于1的正整数, 则不存在 $[1, n)$ 内的数 $k$ 使得 $k*m\%n=0$
    * 证明
        * $m, n$ 互质, 则 $m$ 不能分出一个因子来除 $n$
        * 而 $k$ 比 $n$ 小, 所以想让 $n$ 整除 $km$, 得让 $m$ 分一个因子来和 $k$ 结合(相乘)
* 定理4: 如果 $m,n$ 为大于1的互质的正整数, 则 $r, m+r, m*2+r, ..., m*(n-1)+r$ 这 $n$ 个数对 $n$ 的模余是 $n$ 的一个完全剩余系, 其中 $r$ 为任意大于等于0的整数. 
    * 证明
        * 反证法, 假设这 $n$ 个数中存在两个模 $n$ 同余. 
        * 将它们相减, 可得 $(k * m) \% n = 0$ . $k$ 在小于等于n的范畴内只能取0或 $n$ . 而定理中 $k \in [1, n)$, 所以矛盾. 
